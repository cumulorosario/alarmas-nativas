...
object AlarmNotificationManager {
    ...
    fun createChannels(context: Context) {
        ...
        nm.createNotificationChannel(
            NotificationChannel(CHANNEL_SERVICE, "Servicio Vigia",
        ...
        nm.createNotificationChannel(
            NotificationChannel(CHANNEL_CRITICAL, "Alarmas Críticas",
        ...
        nm.createNotificationChannel(
            NotificationChannel(CHANNEL_WARNINGS, "Advertencias",
    ...
    fun showCriticalAlarm(context: Context, alarm: Alarm) {
        ...
        // Despertar pantalla CORRECTAMENTE: acquire sin release inmediato,
        // lo liberamos después de 10 segundos desde el Handler del main thread
        wakeScreen(context)
        ...
        val fullScreenPi = PendingIntent.getActivity(
            context, alarm.id.id.hashCode(), fullScreenIntent,
            PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE
        )
        ...
        val mainPi = PendingIntent.getActivity(
            context, alarm.id.id.hashCode() + 1, mainIntent,
            PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE
        )
        ...
        val ackPi = PendingIntent.getBroadcast(
            context, alarm.id.id.hashCode() + 2, ackIntent,
            PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE
        )
        ...
        val clearPi = PendingIntent.getBroadcast(
            context, alarm.id.id.hashCode() + 3, clearIntent,
            PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE
        )

        val channel  = if (alarm.isCritical) CHANNEL_CRITICAL else CHANNEL_WARNINGS
        val priority = if (alarm.isCritical) NotificationCompat.PRIORITY_MAX
        ...
        nm.notify(
            NOTIF_ALARM_BASE_ID + alarm.id.id.hashCode().and(0x7FFFFFFF).rem(900),
            notification
        )
    }
