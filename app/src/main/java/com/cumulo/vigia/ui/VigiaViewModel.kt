...
class VigiaViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun login() {
        ...
        viewModelScope.launch {
            ...
            when (val result = repository.login(state.username, state.password, state.rememberMe)) {
                ...
                is Result.Error -> _loginState.update { it.copy(isLoading = false, error = result.message) }
                else -> {}
            }
        ...
        viewModelScope.launch {
            ...
            launch {
                when (val r = repository.getAlarms()) {
                    is Result.Success -> _dashboardState.update {
                        it.copy(
                            // Máximo 20 alarmas, ordenadas por fecha descendente
                            alarms = r.data.take(MAX_ALARMS),
                            isLoading = false,
                            isRefreshing = false,
    ...
    fun acknowledgeAlarm(alarmId: String) {
        viewModelScope.launch {
            ...
            // Cancelar notificación — reconocer = silenciar
